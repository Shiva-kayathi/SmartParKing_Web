...
<div class="container">
  ...
  <!-- query the status of all slots in plot1
  if the status of one slot is true, then change the bg color to green 
  if(true){
  document.getelementbyid(plot1).style.backgroundcolor='green'
  }
  -->
...
*/
	...
	while($i<$countPlot[0])
	{
		...
		$statusQuery= "SELECT COUNT( slot_idslot ) FROM  plot_has_slot WHERE plot_idplot =".$i ." AND STATUS =0";
 		 ...
 		 $countStatus = mysqli_fetch_row($count1);
	     $location ='header("location: detail_plots.php?plot = <?php echo $i ?>")';
		?>
        <form method="post" action="">
        <?php
		if ($countStatus[0]) {
		 
		echo '<a id="btnplot['.$i.']" value ="plot'.$i.'" " style="background-color:green;" name="button['.$i.']"></a>' ;
		}
		else {
		 echo '<a type="button" id="btnplot1" value="plot1"style="background-color:red"></a>';
		}
